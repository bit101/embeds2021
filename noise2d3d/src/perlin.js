...
new HSlider(panel, 20, 40, "Noise Scale", model.scale, 0.002, 0.1)
  .setDecimals(3)
  .bind(model, "scale")
  .addHandler(render);
...
function render(fps) {
  ...
  context1.forEach(res, (x, y) => {
    if (model.color) {
      const h = Noise.perlin2Range(x + model.x, y + model.y, model.scale, 0, 360);
      context1.fillStyle = Color.hsv(h, 1, 1);
    } else {
      const g = Noise.perlin2Range(x + model.x, y + model.y, model.scale, 0, 255);
  ...
  context2.forEach(res, (x, y) => {
    if (model.color) {
      const h = Noise.perlin3Range(x + model.x, y + model.y, z, model.scale, 0, 360);
      context2.fillStyle = Color.hsv(h, 1, 1);
    } else {
      const g = Noise.perlin3Range(x + model.x, y + model.y, z, model.scale, 0, 255);
